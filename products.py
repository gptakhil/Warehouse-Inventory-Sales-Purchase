...
class Product:
    ...
    def __init__(self,root):
      # create object refrence instance of Database class as p
      p = Database()
      p.conn() 
      ...
      '''Create Variables for getting values'''
      pId=StringVar()
      pName=StringVar()
      pPrice=StringVar()
      pQty=StringVar()
      ...
      def close():
        ...
        close = tkinter.messagebox.askyesno("WAREHOUSE INVENTORY \
          SALES PURCHASE MANAGEMENT SYSTEM","Really.... Do you want to close the system")
        if close > 0:
          root.destroy()
      ...
      def insert():
        ...
        if (len(pId.get()) != 0):
          ...
          showInProductList() #called showInProductList method after inserting the data record to database table
        else:
          tkinter.messagebox.askyesno("WAREHOUSE INVENTORY \
            SALES PURCHASE MANAGEMENT SYSTEM","Really .... Enter Product id")
      ...
      def search():
        ...
        print('Product : search method is finished')
      ...
      '''Body frame'''
      OperationFrame = Frame(MainFrame,bd=1,width=1300,height=60,padx=20,pady=20,bg='white',relief=RIDGE)
      ...
      self.txtpId = Entry(LeftBodyFrame,font=('airal',20,'bold'),textvariable=pId,width=30)      
      ...
      self.txtpName = Entry(LeftBodyFrame,font=('airal',20,'bold'),textvariable=pName,width=30)      
      ...
      self.txtpPrice = Entry(LeftBodyFrame,font=('airal',20,'bold'),textvariable=pPrice,width=30)      
      self.txtpPrice.grid(row=2,column=1,sticky=W)
      
      self.labelpQty=Label(LeftBodyFrame,font=('arial',15,'bold'),text='Product Qantity:',padx=2,pady=2,bg='white',fg='blue')
      self.labelpQty.grid(row=3,column=0,sticky=W)
      
      self.txtpQty = Entry(LeftBodyFrame,font=('airal',20,'bold'),textvariable=pQty,width=30)      
      ...
      self.txtpCompany = Entry(LeftBodyFrame,font=('airal',20,'bold'),textvariable=pCompany,width=30)      
      ...
      self.txtpContact = Entry(LeftBodyFrame,font=('airal',20,'bold'),textvariable=pContact,width=30)      
...
class Database:
  def conn(self):
    ...
    cur = con.cursor()
    query="create table if not exists product(pid integer key,\
  ...
  def update(self,pid="",price="",qty="",company="",contact=""):
    ...
    print(pid,"Database : update method finshed\n ")
